...
public class RestServer extends AbstractVerticle {
	...
	private static Double lastValueLuz = 0.0;
	...
	private void addOneSLuz(RoutingContext routingContext) {
		...
		mySqlClient.getConnection(connection ->{
			if(connection.succeeded()) {
				...
				.execute(Tuple.of(idSLuz), res->{
					if(res.succeeded()) {
						lastValueLuz = res.result().iterator().next().getDouble("valor");
					}
					connection.result().close();
				});
				...
				.preparedQuery("INSERT INTO domoticadad.sLuz (idSLuz, valor, tStamp, idPlaca, idGroup) "
						...
						res ->{
							if(res.succeeded()) {
								...
								if(valor<1400.0 && lastValueLuz>1400.0) {
									// Enciende
								...
								}else if(valor>1600.0 && lastValueLuz<1600.0){
									// Apaga
									String topic = "Group" + idGrupo;
